...
class Evosus_Order_Metabox {
    ...
    /**
     * Add Evosus column to orders list (MOVED FROM BELOW)
     */
    public function add_evosus_column($columns) {
        ...
        foreach ($columns as $key => $value) {
            ...
            $('#evosus-validate-btn').on('click', function() {
                var btn = $(this);
                btn.prop('disabled', true).html('<span class="evosus-spinner"></span> Validating...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        btn.prop('disabled', false).text('🔍 Check Order First');
                        
                        if (response.success) {
                            if (response.data.valid) {
                                showMessage('success', '✅ Order validated successfully! Ready to sync.');
                                $('#evosus-validation-info').hide();
                            } else {
                                showMessage('error', '⚠️ Validation issues found. Please refresh to see details.');
                                setTimeout(function() {
                                    location.reload();
                                }, 2000);
                            }
                        } else {
                            showMessage('error', 'Validation failed: ' + response.data.message);
                            $('#evosus-validation-info').hide();
                        }
                    },
                    error: function() {
            ...
            // Sync to Evosus button
            $('#evosus-sync-btn').on('click', function() {
                if (!confirm('Add this order to Evosus?\n\nMake sure you have reviewed the order details.')) {
                    return;
                }
                
                var btn = $(this);
                btn.prop('disabled', true).html('<span class="evosus-spinner"></span> Adding to Evosus...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            showMessage('success', '✅ Order successfully added to Evosus!' + 
                                '<br>Evosus Order ID: <strong>' + response.data.evosus_order_id + '</strong>' +
                                '<br>WC Order #' + response.data.wc_order_number + ' added to PO field');
                            setTimeout(function() {
                                location.reload();
                            }, 3000);
                        } else if (response.data.needs_review) {
                            showMessage('info', '⚠️ Order needs review. Refreshing page...');
                            setTimeout(function() {
                                location.reload();
                            }, 2000);
                        } else {
                            btn.prop('disabled', false).text('➡️ Add to Evosus');
                            showMessage('error', '❌ Failed: ' + response.data.message);
                        }
                    },
                    error: function() {
                        btn.prop('disabled', false).text('➡️ Add to Evosus');
            ...
            // Verify cross-reference button
            $('#evosus-verify-btn').on('click', function() {
                var btn = $(this);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        btn.prop('disabled', false).text('🔍 Verify Cross-Reference');
                        
                        var resultDiv = $('#evosus-verify-result');
                        
                        if (response.success) {
                            if (response.data.verified) {
                                resultDiv.removeClass().addClass('evosus-message-success')
                                    .html('✅ <strong>Verified!</strong><br>' +
                                        'Evosus Order ID: <code>' + response.data.evosus_order_id + '</code><br>' +
                                        'PO Number in Evosus: <code>' + response.data.evosus_po_number + '</code><br>' +
                                        'WC Order Number: <code>' + response.data.wc_order_number + '</code>')
                                    .show();
                            } else {
                                resultDiv.removeClass().addClass('evosus-message-error')
                                    .html('❌ <strong>Mismatch!</strong><br>' +
                                        'PO Number in Evosus: <code>' + response.data.evosus_po_number + '</code><br>' +
                                        'WC Order Number: <code>' + response.data.wc_order_number + '</code><br>' +
                                        'Please check the order in Evosus.')
                                    .show();
                            }
                        } else {
                            resultDiv.removeClass().addClass('evosus-message-error')
            ...
            // Re-validate button
            $('#evosus-revalidate-btn').on('click', function() {
                var btn = $(this);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && response.data.valid) {
                            showMessage('success', '✅ All issues resolved!');
                            setTimeout(function() {
                                location.reload();
                            }, 1500);
                        } else {
                    ...
                    error: function() {
                        btn.prop('disabled', false).text('🔄 Re-check Order');
                        showMessage('error', 'Error checking order.');
                    }
                });
            });
            
            // Approve and sync button
            $('#evosus-approve-sync-btn').on('click', function() {
                if (!confirm('Are you sure all issues are resolved?\n\nThis will add the order to Evosus.')) {
                    return;
                }
                
                var btn = $(this);
                btn.prop('disabled', true).text('Adding to Evosus...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            btn.prop('disabled', false).text('✓ Approve & Add to Evosus');
                            showMessage('error', '❌ Failed: ' + response.data.message);
                        }
                    },
                    error: function() {
            ...
            // Use suggested SKU
            $('.use-sku-btn').on('click', function() {
                var btn = $(this);
                var itemId = btn.data('item-id');
                var newSku = btn.data('new-sku');
                
                btn.prop('disabled', true).text('Mapping...');
                
                mapSku(itemId, newSku, btn);
            });
            
            // Manual SKU mapping
            $('.map-sku-btn').on('click', function() {
                var btn = $(this);
                var itemId = btn.data('item-id');
                var input = $('.manual-sku-input[data-item-id="' + itemId + '"]');
                var newSku = input.val().trim();
            ...
            function mapSku(itemId, newSku, btn) {
                $.ajax({
                    url: ajaxurl,
                    method: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        if (response.success) {
                            btn.text('✓ Mapped').css('background', '#28a745');
                            showMessage('success', 'SKU mapped successfully!');
                        } else {
            ...
            function showMessage(type, message) {
                var messageDiv = $('#evosus-sync-message');
                var className = 'evosus-message-' + type;
